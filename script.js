...
const joinList = evt => {
    //get user entries from text box
    ...
    let isValid = true;
        ...
        //cancel form sumit if any user entries are invalid
        if (!isValid){
            evt.preventDefault();
        }
};
const clearForm = () => {
    ...
    //set focus on first text box after clearing the form
    $("#email_1").focus();
};

//include envent listner for click function
document.addEventListener("DOMContentLoaded", () =>    {
    //hook up click event to both buttons
    $("#join_list").addEventListener("click", joinList);
    $("#clear_form").addEventListener("click", clearForm);

    //set focus onfirst text box after form loads
    $("#email_1").focus();
});